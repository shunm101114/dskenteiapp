...
export const questionsMlAdvanced: Question[] = [
  ...
  },
  {
    ...
    choices: [
      "データをユークリッド距離に基づいて複数のクラスタに分割し、各クラスタの重心を反復的に更新する教師なし学習手法である",
      "説明変数と目的変数の関係を直線（一次式）で表現するモデルである",
      "データを特徴量の条件分岐によりツリー構造で再帰的に分割し、各葉ノードの代表値で予測を行うモデルである",
      "データの確率分布をベイズの定理に基づいて推定し、事後確率が最大となるカテゴリに分類するモデルである",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "連続値の予測に使用される回帰手法で、最小二乗法によりパラメータを推定する",
      "名前に「回帰」とあるが、分類問題に使用される手法である",
      "正解ラベルを必要としない教師なし学習の手法で、データのクラスタ構造を発見する",
      "特徴量の条件分岐により木構造を構築する決定木の一種で、アンサンブル学習にも応用される",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== 教師なし学習基礎 =====
  {
    ...
    choices: [
      "データに正解ラベルを付与し、教師あり学習のための訓練データセットを構築する",
      "データを類似度に基づいてグループに分ける",
      "説明変数と目的変数の関係をモデル化し、連続値の予測を行う回帰分析手法",
      "変数間の因果関係を統計的に推定し、介入効果を定量的に明らかにする",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "データを木構造で階層的に分割し、デンドログラムを用いてクラスタの統合・分割を可視化する手法",
      "クラスタ中心をランダムに配置し、データの割り当てと中心の更新を繰り返す",
      "データの密度が高い領域をコアポイントとして探索し、密度に基づいてクラスタを形成する手法",
      "データ間のユークリッド距離行列を計算し、最短距離のペアを順次結合して階層構造を構築する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "データをk-meansと同様にクラスタに分割し、各グループの特徴を抽出する教師なし学習手法",
      "データの分散が最大となる方向を見つけて次元を削減する",
      "データの欠損値を周囲のデータポイントの統計量（平均値・中央値）を用いて補完する前処理手法",
      "データに正解ラベルを付与して教師あり学習によりクラス分類を行う判別分析手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== 決定木・k-NN =====
  {
    ...
    choices: [
      "データ間のユークリッド距離やマンハッタン距離を計算し、最も近いk個のデータのラベルを多数決で参照する",
      "特徴量に基づいてデータを条件分岐で繰り返し分割し、予測を行う",
      "データの確率分布をガウス分布などのパラメトリックモデルで推定し、ベイズの定理を用いてカテゴリを予測する",
      "重みを持つ結合で複数の層をなすニューラルネットワークを構築し、誤差逆伝播法でパラメータを最適化して予測する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "1つの決定木を非常に深く成長させ、すべての葉ノードが純粋になるまで分割を繰り返す手法",
      "複数の決定木を組み合わせて、多数決や平均で予測するアンサンブル手法",
      "決定木の分岐条件の代わりに線形回帰モデルを各ノードに配置し、区分的線形予測を行う手法",
      "データを事前にk-meansなどでクラスタリングし、各クラスタ内で個別の分類モデルを構築する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "データを特徴量の条件に基づいて木構造で再帰的に分割し、葉ノードのラベルで分類する",
      "新しいデータに最も近いk個の既知データのラベルから多数決で分類する",
      "データの確率分布をガウス分布などのパラメトリックモデルで推定し、ベイズの定理を用いて分類する",
      "多層のニューラルネットワークで特徴を自動抽出し、Softmax関数で各クラスの確率を出力して分類する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "他のどのアルゴリズムよりも常に最も高い予測精度が得られ、ハイパーパラメータの調整も不要である",
      "モデルの判断根拠が可視化でき、解釈しやすい",
      "データ量やフィーチャ数に関係なく大規模データに対して常に高速に動作し、計算コストが一定である",
      "木の深さやノード数を増やしても過学習が起こらず、正則化なしで汎化性能が保たれる",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "モデルがより複雑になり、訓練データのノイズまで学習してしまうため過学習しやすくなる",
      "決定境界がより滑らかになり、ノイズの影響を受けにくくなる",
      "参照する近傍データが増えるため距離計算の最適化が効き、計算時間が大幅に短縮される",
      "kの値に比例して分類精度が単調に向上し、最終的に理論上の最適解に収束する",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== モデル評価基礎 =====
  {
    ...
    choices: [
      "学習時間を短縮するため、データ量を減らしてモデルの収束を早める目的で分割する",
      "未知のデータに対するモデルの汎化性能を正しく評価するため",
      "データの量を2倍に増やすことで、データ拡張と同等の効果を得てモデルの精度を向上させる",
      "異なるアルゴリズムで学習するため、訓練データとテストデータにそれぞれ別のモデルを適用する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "訓練データとテストデータの両方に対して精度が低く、モデルが十分にパターンを捉えられていない状態",
      "訓練データに対して精度が高いが、未知のデータに対して精度が低い状態",
      "モデルの学習が不十分で、エポック数や学習率の調整によりさらに性能が改善する余地がある状態",
      "データが少なすぎて統計的に有意なパターンを学習できず、モデルの予測が不安定な状態",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "全データのうち陽性・陰性を問わず正しく予測できた割合であり、正解率（Accuracy）と同義の指標",
      "陽性と予測したもののうち、実際に陽性だった割合であり、適合率（Precision）と同義の指標",
      "実際に陽性のもののうち、正しく陽性と予測できた割合",
      "適合率と正解率の調和平均として計算される総合的な分類性能指標であり、F1値の変形版である",
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "正解率（Accuracy）と同じ指標であり、全データのうち正しく予測できた割合を計算する",
      "適合率と再現率の調和平均であり、両者のバランスを表す指標である",
      "訓練データに対するモデルの適合度を表す指標で、損失関数の逆数として計算される",
      "1エポックあたりの処理時間からモデルの学習速度を定量化し、効率性を評価する指標である",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== ニューラルネットワーク基礎 =====
  {
    ...
    choices: [
      "複数の隠れ層を持つ深いネットワーク構造で、層が深くなるほど高度な特徴を学習できる",
      "入力に重みを掛けて合計し、閾値で0か1を出力するニューラルネットワークの基本単位",
      "画像認識に特化したネットワーク構造で、畳み込み演算により空間的特徴を捉える",
      "時系列データの処理に特化したネットワーク構造で、再帰的な接続により過去の情報を保持する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "入力データの各特徴量をMin-MaxスケーリングやZ-score正規化により標準的な範囲に変換する",
      "非線形性を導入し、複雑なパターンの学習を可能にする",
      "学習の進行に応じて学習率を自動で減衰させ、勾配降下法の収束を安定化させる",
      "予測値と正解値の差異を二乗誤差やクロスエントロピーなどの損失関数で計算する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "シグモイド関数と同様に、出力値が常に0から1の範囲に収まるように入力を変換する",
      "入力が0以下のとき0を、0より大きいときその値をそのまま出力する",
      "tanh関数と同様に、入力値を-1から1の範囲に非線形変換し、出力の中心を0にする",
      "入力値の絶対値をとることで出力が常に正の値になり、負の勾配を排除する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "外部からの入力データを受け取り、各特徴量をニューロンに割り当てて次の層に渡す層",
      "ネットワークの最終段に位置し、活性化関数を通じて最終的な予測結果を出力する層",
      "入力から出力への間にある中間処理層で、データの特徴を抽出する",
      "予測値と正解値の誤差を損失関数で計算し、逆伝播のための勾配情報を生成する層",
    ],
    correctIndex: 2,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "損失関数の値が最も大きくなる方向にパラメータを更新し、局所最適解を回避する",
      "損失関数の勾配（傾き）を計算し、損失が小さくなる方向にパラメータを更新する",
      "パラメータをランダムに変更しながら最良の組み合わせをモンテカルロ法で探索する手法",
      "すべてのパラメータを解析的に求めた最適値に一度に設定し、反復計算を不要にする手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== CNN・画像認識基礎 =====
  {
    ...
    choices: [
      "画像のサイズを段階的に縮小し、計算量を削減しながら空間情報を圧縮する",
      "フィルタ（カーネル）を用いて画像からエッジや模様などの特徴を抽出する",
      "全結合層と同様に、各ニューロンの出力をSoftmax関数で分類結果の確率として出力する",
      "画像の各チャネルのピクセル値を正規化し、学習時の数値的安定性を向上させる",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "畳み込みフィルタを適用して画像からエッジやテクスチャなどの局所的な特徴を抽出する",
      "特徴マップのサイズを縮小し、計算量の削減と位置ずれへの頑健性を得る",
      "全結合層と連携して最終的な分類結果をクラスごとの確率分布として出力する",
      "データ拡張の一環として入力画像にランダムなノイズを加え、過学習を防止する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "カラー画像は1チャネルのピクセル値で構成され、色情報は各ピクセルの輝度値の範囲（0〜255）で表現される",
      "カラー画像は通常RGB（赤・緑・青）の3チャネルのピクセル値で構成される",
      "画像の解像度はチャネル数によって決まり、チャネル数が多いほど高解像度でピクセル密度も増加する",
      "グレースケール画像は明度・彩度・色相の3チャネルで構成され、各チャネルが異なる階調情報を保持する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "画像全体を1つのカテゴリに分類し、CNNの全結合層から確率分布を出力するタスク",
      "画像内の物体の位置をバウンディングボックスで示し、種類を認識するタスク",
      "画像のピクセルごとにカテゴリを割り当て、セグメンテーションマップとして出力するタスク",
      "画像をテキストで説明するキャプション生成タスクで、CNNとRNNを組み合わせて実現する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "テキストや音声などのデータにも同様に適用でき、汎用性が高いネットワーク構造であるため",
      "画像の局所的な特徴を効率よく捉え、パラメータ数を抑えられるため",
      "全結合層だけのネットワークより常に計算が高速で、GPUを使わなくても十分な速度が得られるため",
      "教師なし学習でもラベルなしの画像データからクラスタリングや特徴抽出に活用できるため",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== RNN・自然言語処理基礎 =====
  {
    ...
    choices: [
      "畳み込み演算により画像の空間的な特徴を効率的に捉え、画像認識に適している",
      "過去の情報を内部状態として保持し、系列データ（時系列やテキスト）を処理できる",
      "教師なし学習におけるクラスタリングに特化したネットワークで、データのグループ構造を学習する",
      "オートエンコーダのように入力データの次元削減を行い、潜在表現を獲得するネットワークである",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "テキストの感情（ポジティブ/ネガティブ）をスコアリングし、センチメント分析を行う処理",
      "日本語などのテキストを意味を持つ最小単位（形態素）に分割する処理",
      "テキストをエンコーダ・デコーダモデルを用いて他の言語に自動翻訳する処理",
      "テキストの文法的な誤りを構文解析ルールに基づいて検出し、修正候補を提示する処理",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "形態素解析の結果に基づき、テキスト中の誤字脱字を自動的に検出して修正する前処理",
      "「は」「の」「です」のような出現頻度が高く情報量の少ない語を除去し、分析の精度を高める",
      "大文字と小文字の表記揺れを統一するため、テキスト中のすべての文字を小文字に変換する正規化処理",
      "テキスト中に含まれる数値や数字表現を除去し、単語のみを対象とした分析を可能にする前処理",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Word2VecやGloVeのように、単語をベクトル空間に埋め込み意味的な類似度を計算する深層学習手法",
      "単語の文書内での重要度を「出現頻度」と「希少性」の両面から評価する手法",
      "コサイン類似度やジャッカード係数を用いて文書同士の類似度を直接計算し、類似文書を検索する手法",
      "固有表現認識（NER）の一種として、テキストから人名・地名・組織名などの固有名詞だけを抽出する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== 生成AI・LLM基礎 =====
  {
    ...
    choices: [
      "畳み込み層を用いた画像認識に特化した小規模なニューラルネットワークモデルで、主に分類タスクに使用される",
      "大量のテキストデータで事前学習された、文章の生成や理解ができる大規模なニューラルネットワークモデル",
      "構造化された数値データの回帰分析に特化したモデルで、時系列予測や需要予測などに主に利用される",
      "音声データの波形をスペクトログラムに変換し、音素認識や話者識別を行う専用のモデル",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "高次元の入力データに対して主成分分析のような次元削減を行い、計算効率を向上させる",
      "入力系列中の各要素が他の要素とどの程度関連するかの重みを計算し、重要な情報に注目する",
      "入力データの特性に応じて教師あり学習と教師なし学習を自動的に切り替えて処理する",
      "学習済みモデルのパラメータ数を削減（プルーニング）し、推論時の計算量を軽減する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "少量のラベル付きデータを使ってモデルの重みパラメータを再学習させるファインチューニング手法",
      "タスクの例を数個プロンプトに含めることで、モデルに期待する出力形式やタスク内容を示す手法",
      "生成コストを抑えるためにモデルの出力トークン数に上限を設定し、応答の長さを制御する手法",
      "多言語対応の精度を高めるために、プロンプトを複数の言語で並列に記述して入力する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "AIが画像生成時に幻想的なエフェクトやアーティファクトを加え、非現実的なビジュアルを出力すること",
      "AIがもっともらしいが事実と異なる情報を生成してしまう現象",
      "AIがトークン数の上限に達して入力を受け付けなくなり、コンテキストウィンドウが飽和する現象",
      "AIの推論時にメモリ使用量が急増し、計算速度が急激に低下してレスポンスが遅延する現象",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "モデルのアーキテクチャと重みをすべて初期化し、ゼロから新しいデータで学習させること",
      "事前学習済みモデルを特定のタスクのデータで追加学習し、性能を調整すること",
      "量子化やプルーニングなどの手法を用いてモデルの推論速度を最適化し、デプロイ効率を高めること",
      "学習率やバッチサイズなどのハイパーパラメータをグリッドサーチ等で手動調整し、精度を向上させること",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== データ前処理 =====
  {
    ...
    choices: [
      "欠損値を含む行や列を削除し、残ったデータのみで学習を行う方法（データ量が十分にある場合に有効）",
      "欠損値を平均値や中央値で補完し、データの分布を大きく変えずに全レコードを学習に使用する方法",
  ...
  },
  {
    ...
    choices: [
      "カテゴリ変数に順序を仮定して0と1の整数値を順番に割り当て、数値データとして扱う手法",
      "カテゴリの各値に対応するバイナリ（0/1）の列を作成し、該当する列のみ1にする手法",
      "Min-Maxスケーリングにより数値データを0から1の範囲に正規化し、特徴量間のスケールを揃える手法",
      "自然言語処理の前処理としてテキストデータをBoW（Bag of Words）形式の数値ベクトルに変換する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "カテゴリ変数をカテゴリ数と同じ数の複数のバイナリ列に展開し、該当列のみ1とする手法",
      "カテゴリ変数の各値に整数を割り当てる手法（例：赤=0, 青=1, 緑=2）",
      "連続的な数値データをビニング処理により離散的なカテゴリデータに変換する手法",
      "連続値を等間隔または等頻度で区間に分割し、各区間にラベルを付けて離散化する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "データの最小値を0、最大値を1として線形変換し、全データを0から1の範囲に収めるMin-Maxスケーリング手法",
      "データの平均を0、標準偏差を1にスケーリングする手法",
      "データの欠損値を平均値や中央値などの統計量を用いて補完し、学習可能な状態にする前処理手法",
      "データをk-meansやDBSCANなどのアルゴリズムを用いて類似度に基づきカテゴリに分割するクラスタリング手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "名義尺度や順序尺度などのカテゴリ変数をワンホットエンコーディング等で数値に変換し、モデルが処理できる形式にするため",
      "特徴量のスケール（単位や範囲）の違いにより特定の特徴量が学習に過大な影響を与えることを防ぐため",
      "データセット内の欠損値を平均値・中央値・最頻値などの統計的手法により補完し、モデルの学習に使用できるようにするため",
      "データ拡張（Data Augmentation）の一種として回転・反転・ノイズ付加により学習サンプル数を増やし、過学習を防止するため",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      "データに正解ラベルが付与されており、入力と出力のペアから直接マッピングを学習する",
      "データの類似度に基づいてクラスタリングを行い、グループ構造を発見する手法である",
      "高次元データの次元削減を行い、情報の損失を最小限に抑えながら特徴を圧縮する",
    ],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      "複数のモデルの予測結果をバギングやブースティングで組み合わせ、最終的な予測精度を向上させる手法",
      "学習データをCSVやJSON等の別のデータ形式にETL処理で変換し、異なるフレームワークで利用可能にする手法",
      "モデルのアーキテクチャとパラメータを異なるプログラミング言語やフレームワークに移植して再実装する手法",
    ],
    correctIndex: 0,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "1つの強力なモデルのみを深い層構造で構築し、そのモデル単体の表現力を最大化して予測を行う手法",
      "複数のモデルを組み合わせて、単一モデルよりも高い予測性能を目指す手法",
      "データをk-meansなどのアルゴリズムで1つのクラスタにまとめ、外れ値を除去してからモデルに学習させる手法",
      "モデルのパラメータ数をプルーニングや量子化により段階的に削減し、推論速度と精度のバランスを最適化する手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "各特徴量から平均を引き標準偏差で割ることで、データの平均を0、標準偏差を1に変換する標準化手法",
      "データを0から1（または任意の範囲）にスケーリングする手法",
      "データに対数変換を適用して右に歪んだ分布を正規分布に近づけ、モデルの学習効率を向上させる手法",
      "データをk-meansなどのクラスタリングアルゴリズムを用いてカテゴリごとにグループ分けする教師なし学習手法",
    ],
    correctIndex: 1,
    explanation:
