...
export const questionsDeAdvanced: Question[] = [
  ...
  },
  {
    ...
    choices: [
      "複数のテーブルをJOIN句で結合し、関連するデータを横方向に結びつけるための構文",
      "SELECT文の中に入れ子にしたSELECT文のこと",
      "CREATE TABLE文を使ってカラム定義やデータ型を指定し、新しいテーブルを作成するための構文",
      "UPDATE文を使って既存のレコードの値を条件付きで変更し、データを更新するための構文",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "一度作成したら要素の追加・変更・削除ができないイミュータブルなデータ構造である",
      "異なるデータ型の要素を混在させることができる",
      "要素の順序は保証されず、挿入順とは異なる順番で格納・返却される",
      "キーと値のペアでデータを格納し、キーを指定して対応する値を取得する構造である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Webアプリケーション開発用のフレームワークで、ルーティングやテンプレートエンジンを内蔵している",
      "多次元配列（ndarray）を使った高速な数値計算を提供するライブラリ",
      "データベースに接続してSQLクエリを実行し、結果をオブジェクトとしてマッピングするORMライブラリ",
      "グラフ描画に特化した可視化ライブラリで、折れ線グラフやヒートマップなどを生成できる",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== データ形式 =====
  {
    ...
    choices: [
      "データをXMLやJSONのようにタグやキーを使って階層構造で表現でき、ネストした親子関係も扱える",
      "カンマなどの区切り文字でデータを区切るテキスト形式で、多くのツールで読み書きできる",
      "セルの書式設定や背景色、条件付き書式、グラフやピボットテーブルなどの情報をファイル内に保持できる",
      "バイナリ形式で保存されるためファイルサイズが小さく圧縮効率が高いが、テキストエディタでは内容を直接読めない",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "CSVと同様に表形式のデータしか表現できず、ネストした構造や配列を扱うことはできない",
      "キーと値のペアによる階層的なデータ構造を表現でき、Web APIのデータ交換でよく使用される",
      "Microsoft Excel専用のファイル形式であり、他のアプリケーションやプログラミング言語では開けない",
      "バイナリ形式で保存されるためファイルサイズが小さく、テキストエディタでは内容を確認できない",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "テキスト形式で保存されるため、ファイルサイズが常に最も小さく圧縮効率にも優れている",
      "複数のシート、セルの書式設定、数式、グラフなどの情報を保持できる",
      "REST APIやGraphQLなどのWeb APIでのデータ交換形式として最も広く採用されている",
      "標準ライブラリだけでプログラムからの読み書きが最も簡単に行え、外部依存が不要である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "日本語や中国語などの東アジア言語専用に設計された文字コードで、英語やラテン文字には対応していない",
      "世界中の文字を扱えるUnicodeの符号化方式で、Webやデータ交換の標準として最も普及している",
      "ASCIIコードの別名であり、アルファベットの大文字・小文字と数字・記号のみを扱う文字コードである",
      "Microsoft Windowsのオペレーティングシステムでのみ使用可能で、LinuxやmacOSでは対応していない",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== ITセキュリティ =====
  {
    ...
    choices: [
      "共通鍵暗号はデータの暗号化のみを行い、公開鍵暗号はデータの復号のみを行うという役割分担がある",
      "共通鍵暗号は暗号化と復号に同じ鍵を使い、公開鍵暗号は暗号化と復号に異なる鍵（公開鍵と秘密鍵）を使う",
      "公開鍵暗号は共通鍵暗号より常に処理速度が速いため、大量のデータを暗号化する場面で優先的に使われる",
      "共通鍵暗号は鍵の配布問題によりインターネット上の通信では使用できず、社内LANでのみ利用される",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "ハッシュ値から元のデータを逆算して復元できる双方向の変換関数であり、データの可逆圧縮にも利用される",
      "入力データから固定長の値を生成し、元のデータへの逆変換ができない一方向の関数である",
      "暗号化と同じ仕組みで、秘密鍵や公開鍵を使ってデータを変換し、鍵がなければ復号できない方式である",
      "同じ入力に対して毎回異なるハッシュ値をランダムに生成することで、推測を困難にするセキュリティ手法である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Webページのリソースをキャッシュし、表示速度とレスポンスタイムを向上させる",
      "通信の暗号化、サーバーの認証、データの完全性の保証を行う",
      "Webサイトのレイアウトやフォントを統一し、ブラウザ間の表示差異を吸収する",
      "ドメイン名をIPアドレスに変換し、ネットワーク上のサーバーの所在を特定する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "覚えやすいように全てのサービスで同じパスワードを使用し、定期的に一括変更することで安全性を維持する",
      "十分な長さと複雑さを持つ一意のパスワードをサービスごとに設定し、パスワードマネージャーで管理する",
      "短くても英数字のみで構成されたパスワードを設定し、30日ごとなど頻繁に変更することで不正利用を防止する",
      "パスワードをExcelやテキストファイルに一覧で記録し、すぐにアクセスできるようデスクトップに保存して管理する",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "入力値の文字数や使用可能な文字種を制限するバリデーションを実装し、不正な入力をフィルタリングする",
      "プレースホルダ（パラメータ化クエリ）を使用してSQL文を組み立てる",
      "HTTPS通信を使用してクライアントとサーバー間のデータを暗号化し、通信経路上での盗聴を防止する",
      "データベース全体をAES等の暗号アルゴリズムで暗号化し、不正アクセス時のデータ漏洩リスクを低減する",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== クラウド基礎 =====
  {
    ...
    choices: [
      "IaaSは完成したソフトウェアをそのまま提供するサービスであり、SaaSは仮想サーバーやネットワークなどのインフラを提供する形態で、両者の管理責任は逆になる",
      "IaaSは仮想サーバーなどのインフラを提供し、PaaSはアプリ開発基盤を提供し、SaaSは完成したソフトウェアをサービスとして提供する",
      "IaaS・PaaS・SaaSは呼称が異なるだけで同じサービス形態であり、提供範囲や利用者側の管理責任に本質的な違いはなくベンダーによって名称が異なるだけである",
      "PaaSはオンプレミス環境でのみ利用可能なサービスモデルであり、AWS・GCP・Azureなどのパブリッククラウド環境での提供は行われていない",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== DB基礎 =====
  {
    ...
    choices: [
      "テーブル内に格納されたデータをAES等の暗号アルゴリズムで自動的に暗号化し、セキュリティを確保する",
      "テーブル内の各レコードを一意に識別するための列（または列の組み合わせ）",
      "テーブル間のデータを結合するためのJOIN条件を定義し、リレーションシップの方向性を決定する制約である",
      "テーブルに格納されたデータの物理的な並び順を決定し、クエリ結果のデフォルトのソート順を指定する列である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "テーブル内のレコードを一意に識別し、重複を許さない主キー制約として機能する",
      "他のテーブルの主キーを参照し、テーブル間の関連を定義する",
      "テーブルのカラムにB-Treeインデックスを自動作成し、検索パフォーマンスを向上させる",
      "テーブルのデータを定期的にスナップショットとして別領域にバックアップする",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "テーブル構造を最適化してインデックスの効果を最大限に引き出し、クエリの実行速度を最大化する",
      "データの重複を排除し、更新時の不整合（更新異常・挿入異常・削除異常）を防ぐ",
      "関連するデータを1つのテーブルにまとめてテーブルの数を最小限にし、JOIN処理を不要にする",
      "カラム単位でデータをAES等の暗号アルゴリズムで暗号化し、不正アクセス時のセキュリティを向上させる",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "カラムに格納されたデータをAES等の暗号アルゴリズムで自動的に暗号化する",
      "特定のカラムに対する検索（SELECT）を高速化する",
      "テーブル間のリレーションを定義し、外部キー制約による参照整合性を保証する",
      "テーブルのデータを定期的にスナップショットとして自動的にバックアップ作成する",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== Web基礎 =====
  {
    ...
    choices: [
      "GETはデータをTLSで暗号化して送信し、POSTは平文のままリクエストボディに含めて送る",
      "GETは主にデータの取得に使い、POSTは主にデータの送信・作成に使う",
      "GETはサーバー側でキャッシュされるため応答が常に高速で、POSTはキャッシュ不可のため常に遅い",
      "GETはHTTPSプロトコルでのみ使用でき、POSTはHTTPプロトコルでのみ使用できる仕様である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "リクエストが成功し、サーバーが正常にレスポンスを返した",
      "リクエストしたリソースが見つからない",
      "サーバー内部で予期しないエラーが発生し、処理を完了できなかった",
      "リクエストしたリソースにアクセスするために認証情報が必要である",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "クライアントとサーバー間のデータ交換には必ずXML形式を使用し、SOAPプロトコルに準拠した通信を行う設計原則である",
      "HTTPメソッド（GET/POST/PUT/DELETE）でリソースに対するCRUD操作を表現し、URLでリソースを識別する",
      "リアルタイム性を確保するため常にWebSocket通信を使用し、サーバーとクライアント間の持続的な双方向接続を維持する設計原則",
      "サーバー側でクライアントのセッション状態をデータベースに永続的に管理し、前回のリクエスト内容を保持した上で次の処理を行う",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "バイナリ形式でシリアライズされるためファイルサイズが小さく、ネットワーク帯域の消費を最小限に抑えられる",
      "人間にも読みやすいテキスト形式で、多くのプログラミング言語で簡単にパースできる",
      "XMLと比較してスキーマ検証やデジタル署名の機能が標準で組み込まれており、セキュリティが高い",
      "Base64エンコードなしで画像や動画のバイナリデータを直接格納でき、マルチメディアの送受信に最も適している",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "ファイルの変更履歴をスナップショットとして保存し、バックアップコピーを作成する機能",
      "開発の履歴を分岐させて、独立した作業を並行して行える仕組み",
      "ローカルリポジトリとリモートリポジトリの間でデータを送受信する通信経路のこと",
      "過去のコミットを指定して変更内容を取り消し、リポジトリを以前の状態に戻す機能",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "異なるファイルを別々のブランチで編集し、それぞれのブランチでコミットした場合",
      "同じファイルの同じ箇所を異なるブランチで別々に変更した場合",
      "ブランチ名が長すぎるか、使用できない特殊文字が含まれている場合",
      "コミットメッセージが空のままコミットを実行し、そのブランチをマージしようとした場合",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Gitリポジトリへのプッシュやプルの権限をユーザーごとに設定し、アクセス制御を行うための設定ファイル",
      "Gitのバージョン管理から除外するファイルやディレクトリのパターンを指定する",
      "Gitのコミットメッセージのフォーマットやテンプレートを定義し、チーム内で統一的なメッセージ規約を適用する",
      "Gitのブランチ名に使用できる文字種や命名規則を定義し、規則に反するブランチの作成を自動的に禁止する",
    ],
    correctIndex: 1,
    explanation:
  ...
  // ===== AutoML/MLOps/プロンプト =====
  {
    ...
    choices: [
      "機械学習モデルの学習からデプロイ、監視まで人間が一切介入せずに完全自動で運用し続けるエンドツーエンドのシステム",
      "特徴量選択、モデル選択、ハイパーパラメータ調整などの機械学習パイプラインを自動化する技術",
      "Webサイトやデータソースから自動的にデータを収集・クレンジングし、学習用データセットを構築するスクレイピングツール",
      "機械学習モデルの推論リクエスト量に応じてサーバーインスタンス数を自動でスケーリングし、レイテンシを最適化する技術",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "ハイパーパラメータの最適化や特徴量選択を自動化することで、機械学習モデルの精度を向上させること",
      "モデルの学習・テスト・デプロイのプロセスを自動化し、迅速かつ安全にモデルを本番環境に届けること",
      "分散処理フレームワークを用いて大量のデータを高速に処理し、学習時間を短縮すること",
      "データの前処理を手動で丁寧に行い、データ品質を担保した上でモデルの学習を実施すること",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "プロンプトに具体的な入出力例を一切含めず、タスクの指示のみを自然言語で与えてモデルの汎化能力に委ねる手法",
      "プロンプトに少数の入出力例を含めることで、AIモデルにタスクのパターンを示す手法",
      "複数のAIモデルを同時に並列で実行し、それぞれの出力を統合してより高精度な結果を得るアンサンブル手法",
      "AIモデルの学習パラメータ数を削減するプルーニングや蒸留を適用して、推論速度を向上させる軽量化手法",
    ],
    correctIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "できるだけ曖昧で短い指示を出す方がAIの創造性と自由度を最大限に引き出せる",
      "役割・タスク・出力形式を明確に指定し、必要に応じて具体例や制約条件を含める",
      "専門用語を多用して複雑なプロンプトを作成し、モデルの高度な言語理解力を活用する",
      "常に英語でプロンプトを書く必要があり、多言語モデルでも英語が最も高精度になる",
    ],
    correctIndex: 1,
    explanation:
