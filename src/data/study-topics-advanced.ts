...
export const studyTopicsAdvanced: StudyTopic[] = [
  ...
  },
  {
    ...
    detail:
      "確率はデータ分析や機械学習の基礎となる重要な概念です。加法定理は「AまたはBが起こる確率」、乗法定理は「AかつBが起こる確率」を求める際に使います。独立事象は一方の結果が他方に影響しない事象（例：コイン投げの各回）であり、排反事象は同時に起こり得ない事象（例：サイコロで1と2が同時に出る）です。条件付き確率は「ある条件が与えられたときの確率」で、ベイズの定理の基礎にもなっています。",
